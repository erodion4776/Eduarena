...
import { Search, ChevronRight, MessageSquare, Brain, Eye, EyeOff, Filter, BookOpen, PlayCircle } from 'lucide-react';
...
export default function KnowledgeHub() {
  ...
  // We use useMemo to cache grouped years so we don't recalculate on every keystroke
  const groupedData = useMemo(() => getGroupedQuestions() || [], []);
  
  const allYears = useMemo(() => {
    return ['All', ...groupedData.map(g => g.year)];
  }, [groupedData]);

  // Filters the master list of questions dynamically based on selected year and search bar input
  ...
  // --- COMMUNICATION WITH THE AI TUTOR ---
  // Pre-fills a message inside the AI Sidebar to kickstart a revision discussion
  const discussWithTutor = (q: JambQuestion) => {
    // Strip out HTML tags from the question for cleaner AI readability
    const cleanQuestionText = q.question.replace(/<[^>]*>?/gm, '');
    const text = `Hi Tutor Chuks, please explain question #${q.id} from the ${q.examyear} Exam. The question asks: "${cleanQuestionText}"`;
    ...
    // Automatically open the side tutor-chat drawer if it isn't open yet
    if (!isChatOpen) toggleChat();
  };

  // Render the interactive Computer Based Test (CBT) Exam simulator view
  if (showSimulator) {
    ...
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden font-sans">
      ...
      <aside className="w-64 border-r border-cyan-500/20 bg-slate-900/50 backdrop-blur-xl p-6 flex flex-col gap-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-bold tracking-tight text-white">Question Hub</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
          {allYears.map((year) => (
            ...
            >
              <span className="font-medium">{year === 'All' ? 'Complete Archive' : `${year} Exam Past Paper`}</span>
              {selectedYear === year && (
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-cyan-950/20 via-slate-950 to-slate-950">
        ...
        {/* HEADER: CBT launchpad & Search system */}
        <header className="p-8 border-b border-white/5 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-1">
              <h1 className="text-3xl font-black tracking-tight text-white uppercase italic">Study Library</h1>
              <p className="text-slate-400 text-sm font-medium tracking-wide">Instant Review Engine. Zero Distractions. Pure Preparation.</p>
            </div>
            
            <div className="flex items-center gap-4 w-full md:w-auto">
              <Button 
                onClick={() => setShowSimulator(true)}
                className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-2xl gap-2 px-6 h-12 shadow-lg shadow-cyan-500/20 group border-none transition-all"
              >
            ...
            {filteredQuestions.length > 0 ? (
              filteredQuestions.map((q, index) => (
                <motion.div
                  ...
                  transition={{ delay: Math.min(index * 0.03, 0.5) }} // Cap delay for fast scrolling responsiveness
                >
                  <Card className="bg-slate-900/40 border border-white/5 hover:border-cyan-500/30 transition-all duration-300 overflow-hidden backdrop-blur-sm group">
                    <CardContent className="p-0">
                      
                      {/* Past Paper Reference Header */}
                      <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Question ID: #{q.id}</span>
                        </div>
                        <Badge className="bg-emerald-500/10 text-emerald-400 border-none text-[10px]">VERIFIED KEY</Badge>
                      </div>

                      <div className="p-8 space-y-6">
                        
                        {/* Reading Comprehension Passage / Instructions (If Available) */}
                        {q.section && (
                          <div className="relative">
                            ...
                            <div className="relative p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
                              <div className="flex items-center gap-2 mb-3 text-cyan-400 font-bold text-[10px] uppercase tracking-widest">
                                <BookOpen className="w-3 h-3" /> Instructions &amp; Reading Context
                              </div>
                              <p 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {Object.entries(q.option || {}).map(([key, value]) => {
                            if (!value || key === 'e') return null; // Ignore invalid options
                        ...
                        <div className="flex flex-wrap items-center gap-4 pt-4">
                          <Button 
                            ...
                            className={`flex-1 md:flex-none h-12 gap-2 rounded-xl font-bold transition-all border-none ${
                          ...
                          >
                            {revealedAnswers[q.id] ? (
                              <><EyeOff className="w-4 h-4" /> Hide Explanation</>
                            ) : (
                              <><Eye className="w-4 h-4" /> Reveal Explanation</>
                            )}
                          </Button>
                          ...
                          <Button 
                            ...
                            className="flex-1 md:flex-none h-12 border-white/10 bg-white/5 hover:bg-white/10 text-white gap-2 rounded-xl group transition-all"
                          >
                            ...
                            >
                              <div className="mt-6 p-6 rounded-2xl bg-white/[0.03] border-l-4 border-emerald-500 space-y-4">
                                <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase tracking-wider">
                                  <Brain className="w-4 h-4" /> Classroom Solution Insight
                                </div>
                                ...
                                <div className="flex gap-4">
                                  <div className="w-12 h-12 shrink-0 rounded-full bg-cyan-500/20 border border-white/10 flex items-center justify-center p-1 overflow-hidden">
                                     <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Chuks" alt="Tutor Avatar representation" />
                                  </div>
                                  <div className="space-y-3">
                                     <div className="p-4 rounded-tr-2xl rounded-br-2xl rounded-bl-2xl bg-slate-800 text-slate-200 text-sm leading-relaxed">
                                       {q.solution || "This follows standard grammatical rules for this syllabus category. Feel free to open a tutor dialogue to review this question step-by-step!"}
                                     </div>
                                     <div className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-2">
                                       Correct Answer Choice: 
                                       <span className="text-emerald-400 text-xl font-black">{q.answer?.toUpperCase()}</span>
                                     </div>
                                  </div>
                                </div>
                              </div>
              ...
              <div className="py-40 text-center space-y-6">
                ...
                <div>
                   ...
                   <p className="text-slate-500 max-w-sm mx-auto mt-2">Adjust your search or look into a different class year category inside the syllabus archive.</p>
                </div>
                <Button onClick={() => { setSearch(''); setSelectedYear('All'); }} variant="link" className="text-cyan-400">
